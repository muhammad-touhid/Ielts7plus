  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-6">
      ...
      <div className="mt-8 divide-y divide-gray-200 border border-gray-200 rounded-lg overflow-hidden">
        {pages.length === 0 && (
          <p className="p-6 text-sm text-gray-500">No pages yet. Create your first one above.</p>
        )}
        {pages.map((page) => (
          <div key={page.id} className="flex items-center justify-between p-4 bg-white">
            <div>
            ...
            <div className="flex gap-3">
              <Link href={`/admin/pages/${page.id}/edit`} className="text-blue-600 hover:underline text-sm font-medium">
                Edit
              </Link>
              {page.status === "published" && (
                <Link href={`/p/${page.slug}`} target="_blank" className="text-gray-500 hover:underline text-sm">
                  View
                </Link>
              )}
            </div>
